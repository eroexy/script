...
--// Ultra-Realistic WalkSpeed + Infinite Jump + JumpPower (Rayfield Integrated)
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local UserInputService = game:GetService("UserInputService")
...
-----------------------------------------------------
-- REALISTIC WALKSPEED FUNCTION
-----------------------------------------------------

local function enableRealisticWalkSpeed()
	...
	wsConnection = RunService.Stepped:Connect(function()
		...
		local newVel = moveDir * walkSpeed
		newVel = Vector3.new(newVel.X, hrp.Velocity.Y, newVel.Z)
...
local function enableInfiniteJump()
	...
	ijConnection = UserInputService.JumpRequest:Connect(function()
		...
		local hum = char:FindFirstChildOfClass("Humanoid")
		if hum then
			-- Apply JumpPower manually each jump
...
-----------------------------------------------------
-- FIX: Update Jump Power on Humanoid spawn
-----------------------------------------------------
...
-----------------------------------------------------
-- RAYFIELD UI CONNECTION
-----------------------------------------------------

local ToggleWS = Tab:CreateToggle({
	Name = "Walkspeed",
	CurrentValue = false,
	Flag = "WalkSpeed",
	Callback = function(Value)
...
local ToggleIJ = Tab:CreateToggle({
	Name = "Infinite Jump",
	CurrentValue = false,
	Flag = "InfJump",
	Callback = function(Value)
